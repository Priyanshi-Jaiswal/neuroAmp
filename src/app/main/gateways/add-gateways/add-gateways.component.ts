...
@Component({
  selector: 'app-add-gateways',
  templateUrl: './add-gateways.component.html',
  styleUrls: ['./add-gateways.component.scss']
})
export class AddGatewaysComponent implements AfterViewInit, OnDestroy {
  ...
  loadGatewayData(id: string): void {
    this.appService.getSingleGateway(id).subscribe({
      next: (response) => {
        ...
        // Call initMap() after data is loaded and then reverse geocode.
        this.initMap();
  ...
  createGateway(): void {
    ...
    console.log("Created data is: ", gatewayData)

    this.appService.createGateway(gatewayData).subscribe({
      next: (response) => {
